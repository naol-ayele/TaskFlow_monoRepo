import "@taskflow/ui/globals.css";
import Link from "next/link";
import {
  Home,
  CheckSquare,
  FileText,
  Timer,
  MessageCircle,
  BarChart,
} from "lucide-react";
import { TimerProvider } from "./timer-context";

export const CURRENT_USER_ID = "user_member4";
export const CURRENT_USER_NAME = "Member 4";

const navItems = [
  { href: "/", label: "Dashboard", icon: Home },
  { href: "/tasks", label: "Tasks", icon: CheckSquare },
  { href: "/notes", label: "Notes", icon: FileText },
  { href: "/timer", label: "Timer", icon: Timer },
  { href: "/timer/chart", label: "Weekly Chart", icon: BarChart },
  { href: "/chat", label: "Chat", icon: MessageCircle },
];

export default function RootLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <html lang="en">
      <body>
        <div className="h-screen overflow-hidden bg-[var(--background)]">
          <nav className="fixed left-0 top-0 h-screen w-60 border-r border-[var(--border)] bg-[var(--surface)] p-4">
            <div className="mb-6 text-xl font-bold text-[var(--accent)]">
              TaskFlow
            </div>
            <div className="flex flex-col gap-2">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center gap-3 rounded-md px-3 py-2 text-sm text-[var(--text-muted)] hover:bg-[var(--surface-raised)] hover:text-[var(--text-primary)]"
                >
                  <item.icon className="h-5 w-5" />
                  {item.label}
                </Link>
              ))}
            </div>
          </nav>
          <main className="ml-60 h-screen flex flex-col">
            <header className="flex-shrink-0 px-6 pt-6 pb-4">
              <h1 className="text-lg font-semibold text-[var(--text-primary)]">
                {CURRENT_USER_NAME}'s Dashboard
              </h1>
            </header>
            <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-6">
              <TimerProvider>{children}</TimerProvider>
            </div>
          </main>
        </div>
      </body>
    </html>
  );
}
